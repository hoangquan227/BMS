    ...
    return (
      <div className="flex flex-wrap gap-1">
        {(row.tags || [value]).slice(0, 3).map((tag) => (
          <span className="rounded-bms-pill bg-blue-50 px-2 py-1 text-xs font-bold text-bms-primary" key={tag}>
            {tag}
          </span>
        ))}
      </div>
    ...
    <section className="overflow-hidden rounded-[14px] border border-bms-border bg-white">
      <div className="flex flex-col gap-3 border-b border-bms-border bg-slate-50 px-4 py-3 xl:flex-row xl:items-center xl:justify-between">
        <div className="flex flex-wrap items-center gap-3">
          <button className="rounded-bms-pill bg-green-50 px-3 py-2 text-sm font-black text-bms-success" type="button">
            Lọc
          </button>
          <button className="rounded-bms-pill border border-bms-border bg-white px-4 py-2 text-sm font-black text-slate-900" type="button">
            {module.filterLabel}
          </button>
          <input
            className="min-h-10 w-full min-w-[220px] rounded-bms-pill border border-bms-border px-4 text-sm font-semibold outline-none focus:border-bms-primary sm:w-[320px]"
            placeholder="Tìm kiếm trong danh sách"
          />
        </div>

        <div className="flex flex-wrap items-center gap-2">
          <button className="rounded-bms-pill border border-bms-border bg-white px-3 py-2 text-sm font-black text-slate-700" type="button">
            Dạng bảng
          </button>
          <button className="rounded-bms-pill bg-bms-success px-4 py-2 text-sm font-black text-white" type="button">
            + {module.createLabel}
          </button>
        </div>
      </div>
      ...
      <div className="overflow-auto">
        <table className="w-full min-w-[980px] text-left text-sm">
          <thead className="border-b border-bms-border bg-white text-xs uppercase text-slate-700">
            <tr>
      ...
      <footer className="flex flex-col gap-2 border-t border-bms-border bg-white px-4 py-2 text-xs font-bold text-slate-800 lg:flex-row lg:items-center lg:justify-between">
