      ...
      <div className="space-y-6 p-5">
        {rows.map((item) => (
          <article className="grid grid-cols-[82px_1fr] gap-4" key={`${item.time}-${item.title}`}>
            <p className="text-[13px] font-bold text-slate-600">{item.time}</p>
            ...
            <tr className="hover:bg-blue-50/60" key={index}>
              {columns.map((column) => (
                <td className="px-4 py-3 font-semibold text-slate-900" key={column}>
                  {column === "status" ? <StatusBadge className="text-[13px]" status={row[column]} /> : row[column]}
                </td>
              ))}
            </tr>
          ))}
    ...
    <section className="overflow-hidden rounded-[14px] border border-bms-border bg-white">
      <header className="flex flex-col gap-3 border-b border-bms-border bg-white px-4 py-3 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          {quickAdds.slice(0, 4).map((label) => (
            <button className="rounded-bms-pill border border-bms-success px-3 py-2 text-[13px] font-extrabold text-bms-success transition hover:bg-green-50" key={label} type="button">
              {label}
            </button>
          ))}
        </div>
      ...
      <div className="grid min-h-[620px] lg:grid-cols-[360px_1fr]">
        <aside className="border-r border-bms-border bg-white p-4">
          ...
          <section className="mb-5">
            <h3 className="mb-3 text-[15px] font-extrabold text-slate-950">Tag</h3>
            <div className="flex flex-wrap gap-2">
              {(record.tags || ["Khách hàng"]).map((tag) => (
                <span className="rounded-bms-pill bg-blue-50 px-3 py-1 text-[13px] font-extrabold text-bms-primary" key={tag}>
                  {tag}
                </span>
              ))}
              <button className="rounded-bms-pill bg-slate-100 px-3 py-1 text-[13px] font-extrabold text-slate-700" type="button">
                + Tag
              </button>
            </div>
          </section>
          ...
          <section>
            ...
            <div className="grid gap-2">
              {quickAdds.map((label) => (
                <button className="rounded-[10px] border border-bms-border bg-slate-50 px-3 py-2 text-left text-[14px] font-bold text-slate-800 transition hover:border-bms-primary hover:bg-blue-50 hover:text-bms-primary" key={label} type="button">
                  {label}
                </button>
              ))}
            </div>
          ...
          <div className="p-4">
            <div className="overflow-hidden rounded-[12px] border border-bms-border bg-white">
              <div className="flex items-center justify-between border-b border-bms-border px-4 py-3">
                <h3 className="text-[16px] font-extrabold text-slate-950">{activeTab}</h3>
                <button className="rounded-bms-pill border border-bms-border px-3 py-1.5 text-[13px] font-extrabold text-slate-700 hover:bg-slate-50" type="button">
                  Lọc: Tất cả
                </button>
              </div>
